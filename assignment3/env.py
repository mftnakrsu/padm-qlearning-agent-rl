...
class ContinuousMazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        - Optimized reward structure for DQN training:
          * Small step penalty to encourage efficiency
          * Negative reward for wall collisions
          * Large negative reward for danger zones
          * Positive reward for reaching goal
          * Distance-based shaping to guide agent toward goal
        """

        # Initialize reward with small step penalty (encourages efficiency)
        reward = -0.01
        ...
        for (xmin, ymin, xmax, ymax) in self.danger_zones:
            if xmin <= self.agent_pos[0] <= xmax and ymin <= self.agent_pos[1] <= ymax:
                # Danger zone: large negative reward
                reward = -100.0
                done = True
                break
